...
export default function AdminPreviewPage() {
  ...
  // Render mobile version if on mobile device or if mobile/tablet preview is requested
  if (isMobile || (isPreviewMode && (previewDevice === 'mobile' || previewDevice === 'tablet'))) {
    return (
      <>
        <Head>
          <title>Admin Login Preview - Pinnacle Thrive Coaching</title>
          <meta name="viewport" content={previewDevice === 'tablet' ? "width=768, initial-scale=1" : "width=device-width, initial-scale=1"} />
  ...
  useEffect(() => {
    if (isPreviewMode) {
      ...
      // Set viewport meta tag for device-specific preview
  ...
  // Render desktop version with device preview and mobile preview toggle
  return (
    <>
      <Head>
        <title>Admin Login Preview - Pinnacle Thrive Coaching</title>
      ...
      {isPreviewMode ? (
        // Preview mode - render with device-specific styling
        <div className={`${previewDevice}-preview-container`}>
          {/* Back to Admin Button */}
          <div className="fixed top-4 left-4 z-50">
            <button
              onClick={() => {
                // Remove preview parameters and go back to admin
        ...
        // Normal mode - render with device preview controls
        <MobilePreviewToggle>
          <DevicePreview>
